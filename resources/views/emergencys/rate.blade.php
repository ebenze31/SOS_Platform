...
<div class="w-full md:h-[calc(100%-71.75px)] relative flex justify-center sm:items-center p-3 bg-slate-50 dark:bg-slate-900" style="height: calc(100vh - 71.75px); margin-top:71.75px; overflow: auto;">
    ...
    <div class="relative w-full max-w-[500px] bg-white dark:bg-[#1a2632] rounded-xl shadow-xl flex flex-col overflow-auto ring-1 ring-black/5 dark:ring-white/10">
        ...
        <header class="px-8 py-6 border-b border-slate-100 dark:border-slate-700/50 flex items-center justify-between bg-white dark:bg-[#1a2632]">
            <div>
                <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">ประเมินการบริการ</h1>
                <p class="text-xs text-slate-400 mt-1 uppercase tracking-wide">
                    รหัสเคส: {{ $operation->operating_code ?? 'ไม่ระบุ' }}
                </p>
            </div>
            <a href="{{ url('/') }}" class="h-10 w-10 rounded-full bg-slate-50 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-500 dark:hover:bg-slate-700 flex items-center justify-center transition-colors">
        ...
        <div class="flex flex-col bg-white dark:bg-[#1a2632]">
            ...
            <section class="px-8 pt-8 pb-2 text-center">
                ...
                <h2 class="mt-3 text-lg font-bold text-slate-900 dark:text-white">
                    {{ $officer->name_officer ?? 'เจ้าหน้าที่' }}
                </h2>
            ...
            <section class="px-6 py-6 border-t border-slate-50 dark:border-slate-800">
                <h3 class="text-center text-base font-semibold text-slate-700 dark:text-slate-200 mb-6">ความพึงพอใจด้านระยะเวลา</h3>
                <div class="flex justify-between items-end px-2 gap-2">
                    <button onclick="selectTimeRating(1)" class="time-btn group flex flex-col items-center gap-2 transition-all hover:-translate-y-1" data-rating="1">
                        ...
                        <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wide group-hover:text-red-500 transition-colors">ช้ามาก</span>
                    </button>
                    <button onclick="selectTimeRating(2)" class="time-btn group flex flex-col items-center gap-2 transition-all hover:-translate-y-1" data-rating="2">
                        ...
                        <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wide group-hover:text-orange-500 transition-colors">ช้า</span>
                    </button>
                    ...
                    <button onclick="selectTimeRating(4)" class="time-btn group flex flex-col items-center gap-2 transition-all hover:-translate-y-1" data-rating="4">
                        ...
                        <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wide group-hover:text-lime-600 transition-colors">รวดเร็ว</span>
                    </button>
                    <button onclick="selectTimeRating(5)" class="time-btn group flex flex-col items-center gap-2 transition-all hover:-translate-y-1" data-rating="5">
                        ...
                        <span class="text-[10px] font-medium text-slate-400 uppercase tracking-wide group-hover:text-green-600 transition-colors">ไวมาก</span>
                    </button>
                </div>
            </section>
            ...
            <section class="px-8 pb-4">
                <div class="relative">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2" for="feedback">คำแนะนำ / ติชม (ไม่บังคับ)</label>
                    <textarea class="w-full rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white placeholder-slate-400 focus:border-primary focus:ring-primary dark:focus:border-blue-500 dark:focus:ring-blue-500 text-sm p-3 min-h-[100px] resize-none" id="feedback" placeholder="คำแนะนำของท่านช่วยให้เราสามารถพัฒนาการบริการให้ดียิ่งขึ้นได้"></textarea>
                </div>
            </section>

        </div>
        <footer class="px-8 py-6 bg-white dark:bg-[#1a2632] border-t border-slate-100 dark:border-slate-700">
            <button onclick="submitRating()" id="submitBtn" class="w-full flex items-center justify-center gap-2 px-4 py-3.5 text-sm font-bold text-white bg-primary hover:bg-blue-700 rounded-lg shadow-lg shadow-blue-500/20 transition-all transform active:scale-[0.98]">
                <span id="submitBtnText">ยืนยันการประเมิน</span>
                <span id="submitBtnIcon" class="material-symbols-outlined text-[18px]">arrow_forward</span>
            </button>
        </footer>
    </div>
</div>
...
<script>
    ...
    async function submitRating() {
        ...
        if (ratingData.timeRating === null) {
            alert('กรุณาเลือกคะแนนความพึงพอใจด้านระยะเวลา');
            return;
        }
        ...
        submitBtnText.innerText = 'กำลังบันทึก...';
        submitBtnIcon.innerText = 'sync';
        submitBtnIcon.classList.add('animate-spin');

        // ข้อมูลที่จะส่งให้ Controller
        const payload = {
            score_impression: ratingData.satisfactionRating,
            score_period: ratingData.timeRating,
            comment_help: ratingData.feedback,
            _token: '{{ csrf_token() }}'
        };

        try {
            const response = await fetch('{{ route("emergency.rate.submit", $emergency->id) }}', {
                method: 'POST',
                headers: {
    ...
    function resetBtn() {
        ...
        submitBtnText.innerText = 'ยืนยันการประเมิน';
        submitBtnIcon.innerText = 'arrow_forward';
    ...
    function returnHome() {
        window.location.href = '{{ url("/sos/history") }}';
    }
</script>
